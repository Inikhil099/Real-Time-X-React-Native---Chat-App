    ...
    >
      <View className="relative">
        <View className="items-center mt-10">
          <View className="relative">
            <View className="rounded-full border-4 border-[#4C1D95]">
              <Image
                source={user?.imageUrl}
                style={{ width: 100, height: 100, borderRadius: 999 }}
              />
            </View>
            <Pressable className="absolute bottom-1 right-1 w-8 h-8 bg-[#4C1D95]  rounded-full items-center justify-center border-2 border-surface-dark">
        ...
        <View key={section.title} className="mt-6 mx-5 ">
          ...
          <View className="bg-surface-card rounded-2xl overflow-hidden">
            {section.items.map((item, i) => (
              <Pressable
                className={`flex-row items-center px-4 py-3.5 active:bg-surface-light ${i < section.items.length - 1 ? "border-b border-surface-light" : ""}`}
                key={item.label}
              >
                <View
